...
const GalleryTabs = ({
  ...
  return <motion.div className="mb-12" initial={{
    opacity: 0,
    y: 20
  }} animate={{
    opacity: 1,
    ...
    delay: 0.3
      ...
      <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        {categories.map(category => <TabsContent key={category} value={category} className="pt-6">
            <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" variants={containerVariants} initial="hidden" animate="visible">
              {filterImages(category).map(image => <GalleryImageCard key={image.id} image={image} onClick={onImageClick} />)}
            </motion.div>
          </TabsContent>)}
      </Tabs>
    </motion.div>;
