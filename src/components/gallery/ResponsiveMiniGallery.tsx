...
const ResponsiveMiniGallery = ({
  ...
  // Plugin for autoplay
  const plugin = autoplay ? [Autoplay({
    delay: 4000,
  ...
  return <div className={cn("w-full", className)}>
      <div className="relative">
        ...
        loop: true
          ...
          <CarouselContent>
            {displayImages.map(image => <CarouselItem key={image.id} className="md:basis-1/2 lg:basis-1/3 px-2">
                <div className="overflow-hidden rounded-xl shadow-lg transition-all duration-300 hover:shadow-2xl group cursor-pointer" onClick={() => onImageClick && onImageClick(image.id)}>
        ...
        <div className="flex justify-center mt-6 gap-2">
          {displayImages.map((image, index) => <div key={`thumb-${image.id}`} className="w-12 h-1.5 rounded-full bg-mun-purple/30 overflow-hidden">
