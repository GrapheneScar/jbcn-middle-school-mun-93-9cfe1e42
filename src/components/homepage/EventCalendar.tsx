  ...
  return <motion.div initial={{
    ...
    duration: 0.5
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
        <div className="w-full md:w-auto flex justify-center bg-black/40 rounded-xl p-2 border border-mun-purple/20">
          <Calendar mode="single" selected={date} onSelect={setDate} className="p-0 pointer-events-auto" modifiers={{
        ...
        }} defaultMonth={conferenceStartDate} footer={<div className="text-center text-xs text-white/70 mt-2">
                April 25-26, 2025
              </div>} />
        </div>
        ...
        <div className="text-left flex-grow w-full md:w-auto">
          <div className="space-y-3">
            {importantDates.map((item, index) => <div key={index} className="flex items-start">
                <div className={`w-3 h-3 mt-1.5 rounded-full ${format(item.date, 'yyyy-MM-dd') === format(date || conferenceStartDate, 'yyyy-MM-dd') ? 'bg-mun-purple-light' : 'bg-white/30'}`} />
                <div className="ml-3">
          ...
          <div className="mt-4 pt-3 border-t border-white/10">
            <p className="text-white/80 text-sm">Mark your calendar for JBCN Parel Middle School MUN 2025! Join us for two days of debate, diplomacy, and global solutions.</p>
          </div>
        </div>
      </div>
    </motion.div>;
