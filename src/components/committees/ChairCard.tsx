...
const ChairCard = ({
  ...
  return <motion.div initial={{
    ...
    duration: 0.5
      ...
      <div className="glass-panel p-6 rounded-xl overflow-hidden border border-mun-purple/20">
        <div className="flex flex-col items-center md:flex-row md:items-start gap-6">
          <div className="w-36 h-36 rounded-full overflow-hidden flex-shrink-0 border-2 border-mun-purple/30 bg-mun-purple/20">
            {chair.image ? <img src={chair.image} alt={`${chair.name} profile`} className="w-full h-full object-cover" onError={e => {
          ...
          <div className="flex-grow text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-4">
              {chair.linkedin && <a href={chair.linkedin} target="_blank" rel="noopener noreferrer" className="text-sm">
              ...
              {chair.cv && <a href={chair.cv} target="_blank" rel="noopener noreferrer">
                  <Button variant="outline" size="sm" className="bg-white/5 hover:bg-white/10 border-mun-purple/30">
      ...
      <ChairGalleryModal isOpen={isGalleryOpen} onClose={() => setIsGalleryOpen(false)} chairName={chair.name} />
    </motion.div>;
};
