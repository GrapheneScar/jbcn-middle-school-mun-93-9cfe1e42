  ...
  const startEasterEggTimer = (chair: any) => {
    ...
    const timer = setTimeout(() => {
      ...
      const easterEggEvent = new CustomEvent('easterEggTriggered', {
        detail: { title: chair.title || 'Department Head' }
      });
      ...
      <div className="flex flex-wrap gap-8 justify-center max-w-4xl mx-auto">
        {department.chairs.map((chair, chairIndex) => {
          ...
          return (
            ...
            >
              <div 
                className={`glass-panel p-4 overflow-hidden transition-transform duration-300 ${activeEasterEggChair === chair.name ? 'glow-effect' : ''}`}
                onMouseDown={() => { pressTimer = startEasterEggTimer(chair) }}
                onMouseUp={() => { if (pressTimer) clearTimeout(pressTimer) }}
                onMouseLeave={() => { if (pressTimer) clearTimeout(pressTimer) }}
                onTouchStart={() => { pressTimer = startEasterEggTimer(chair) }}
              ...
              >
                <div className="flex flex-col items-center">
                  ...
                  {hasEasterEgg && (
                    <p className="text-white/30 text-xs italic mt-1 mb-2">Hold for a surprise</p>
                  )}
                </div>
              </div>
