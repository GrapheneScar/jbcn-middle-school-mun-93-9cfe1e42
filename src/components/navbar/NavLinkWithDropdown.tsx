...
const NavLinkWithDropdown = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target as Node) && linkRef.current && !linkRef.current.contains(event.target as Node)) {
        setIsOpen(false);
      }
    };
  ...
  return <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      {/* Main link */}
      <div ref={linkRef}>
        <div onClick={handleClick} className={`px-3 py-2 text-sm font-medium rounded-md transition-colors relative group cursor-pointer ${isActive(item.path) ? "text-white" : "text-gray-300 hover:text-white"}`}>
          <span className="flex items-center text-base font-semibold">
            {item.name}
            {submenuItems && submenuItems.length > 0 && <ChevronDown className={`ml-1 h-4 w-4 transition-transform ${isOpen ? "rotate-180" : ""}`} />}
          </span>

          {/* Highlight pill for active state */}
          {isActive(item.path) && <span className="absolute inset-0 bg-mun-purple/20 rounded-md -z-10"></span>}
        </div>
      </div>

      {/* Dropdown Menu */}
      {submenuItems && submenuItems.length > 0 && <div ref={dropdownRef}>
          <DropdownMenu isOpen={isOpen} submenu={submenuItems} isActive={isActive} closeMenu={closeMenu} />
        </div>}
    </div>;
};
